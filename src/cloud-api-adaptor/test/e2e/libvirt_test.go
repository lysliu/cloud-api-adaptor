...
func TestLibvirtKbsKeyRelease(t *testing.T) {
	...
	if isTestWithKbsIBMSE() {
		...
		DoTestIBMSEInitdataEmpty(t, testEnv, assert)
		DoTestIBMSEInitdata(t, testEnv, assert)
		...
		DoTestIBMSEInitdataPolicy(t, testEnv, assert)
...
func TestLibvirtIBMSEInitdata(t *testing.T) {
	if !isTestWithKbs() {
		t.Skip("Skipping kbs related test as kbs is not deployed")
	}
	_ = keyBrokerService.SetSampleSecretKey()
